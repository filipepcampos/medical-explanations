...
class ChexpertSmall(Dataset):
    ...
    def _maybe_process(self, data_filter):
        ...
        test_df = pd.read_csv(os.path.join(self.root, 'CheXpert-v1.0-small', 'valid.csv'), keep_default_na=True)
        train_df = self._load_and_preprocess_training_data(os.path.join(self.root, 'CheXpert-v1.0-small', 'train.csv'), data_filter)
